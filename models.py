...
class IMLE(nn.Module):
    def __init__(self, H):
        super().__init__()
        self.decoder = UNet2DModel(
            sample_size=32,             # Height and width of the input image
            in_channels=4,               # RGB
            out_channels=4,              # Usually same as in_channels (for predicting noise)
            layers_per_block=2,
            block_out_channels=(128, 128, 256, 256, 512, 512),  # Controls depth and size
            down_block_types=(
                "DownBlock2D", "DownBlock2D", "DownBlock2D",
    ...
    def forward(self, latents, input_is_w=False):
        latent_reshaped = latents.view(latents.shape[0], 4, 32, 32)
