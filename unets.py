...
class OutputNorm(torch.nn.Module):
    def __init__(self, num_channels, target_std=1.0, eps=1e-6):
        super().__init__()
        self.log_weight = torch.nn.Parameter(torch.zeros(num_channels))
        self.bias = torch.nn.Parameter(torch.zeros(num_channels))
        self.eps = eps
    ...
    def forward(self, x):
        ...
        x = (x - mean) / std
        w = torch.exp(self.log_weight).reshape(1,-1,1,1)
        return x * (self.target_std * w) + self.bias.reshape(1,-1,1,1)
...
class SongUNet(torch.nn.Module):
    def __init__(self,
        ...
        **kwargs
        ...
        init_zero = dict(init_mode='xavier_uniform')
        ...
        self.out_norm_final = OutputNorm(out_channels, target_std=1.0)  # e.g. sigma=1.0
